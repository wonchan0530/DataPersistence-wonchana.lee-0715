...
## 추가 리뷰 — Phase 8 이후 코드 재점검

Phase 0~8 완료 후 요청에 따라 코드 전체를 다시 리뷰하며 두 가지를 발견/수정함.

### 발견 1: 콘솔 메뉴 EOF 무한 루프 (버그, 우선순위 높음)

- **증상**: `SampleMenu::run()`/`OrderMenu::run()`은 `ConsoleIO::readLine()`을 사용하는데, `readLine`은
  표준입력이 끊겨도(EOF) 예외 없이 빈 문자열 `""`을 반환한다. 서브메뉴 루프는 `""`을 "잘못된 입력"으로
  처리하고 다시 루프를 도는데, EOF 이후에는 매번 즉시 또 EOF가 나므로 **루프가 절대 끝나지 않고
  CPU를 점유하며 폭주**한다.
- **재현**: `printf '1\n' | ./build/DataPersistenceApp.exe` (시료 관리 메뉴만 선택하고 종료 코드를 주지
  않음) → 2초 안에 3,140,008줄 출력. 실제 서비스에서는 입력이 파일/파이프에서 끊기거나 터미널이
  비정상 종료될 때 그대로 재현될 수 있는 실사용 시나리오임.
- **수정**: `main.cpp`가 이미 최상위 루프에서 `if (!std::getline(...)) break;`로 EOF를 감지하고 있던
  것과 동일한 방식으로, `SampleMenu::run()`/`OrderMenu::run()`에도 `readLine()` 직후 `if (!std::cin) break;`
  체크를 추가해 상위 메뉴로 안전하게 복귀하도록 함.
- **검증**: 동일한 재현 명령이 이제 0.13~0.18초 만에 11줄 출력 후 정상 종료됨을 확인.

### 발견 2: `ConsoleIO`의 숫자 파싱 로직 4회 반복 (Rule of Three)

- `readInt`/`readDouble`/`readOptionalInt`/`readOptionalDouble` 4개 함수가 "try `std::stoi`/`std::stod` →
  전체 문자열이 소비됐는지 확인 → 실패 시 `nullopt`"라는 동일한 블록을 각자 들고 있었음(3회를 넘어 4회
  반복 — 명백한 리팩토링 대상).
- `detail::parseFull<T>(line, parseFn)` 템플릿 헬퍼로 통합하고, `detail::parseInt`/`parseDouble`을 거쳐
  4개 공개 함수는 각각 한 줄로 위임하도록 정리함. 동작(전체 소비 검증, 예외 시 `nullopt`)은 그대로 유지.

### 검증

- `build/`, `data/`, `test_data/`를 모두 지운 뒤 클린 빌드 → `/W4` 경고 0건.
- `ctest --test-dir build --output-on-failure` → `DataPersistenceTests` 26/26 체크 통과.
- 콘솔 앱으로 시료 등록(정상) → 시료 등록(숫자 오입력으로 취소) → 시료 부분 수정(재고만 999로 변경) →
  주문 등록(정상) → 주문 등록(존재하지 않는 시료 참조로 실패) → 조회, 재시작 후 `data/samples.json`
  (재고 999)·`data/orders.json` 데이터가 그대로 유지됨을 확인 — 리팩토링 전후로 동일하게 동작.
- EOF 무한 루프 재현 명령을 다시 실행해 더 이상 폭주하지 않음을 확인 (위 발견 1 참고).

**결론**: 실사용 시 CPU를 100% 점유할 수 있었던 무한 루프 버그를 수정하고, 숫자 입력 파싱의 반복 코드를
정리함. 전체 빌드/테스트/수동 시나리오가 모두 정상임을 재확인.
